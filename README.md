...
dataset/*.csv ──► intake.py    join, dated FX, status rules, recurrence + income streams, evidence facts
              ...
              ──► explain.py   Groq allam-2-7b (POC; BUYORWAIT_EXPLAIN_MODEL overrides) writes decision_explanation from the decision packet (template fallback, 429 backoff)
