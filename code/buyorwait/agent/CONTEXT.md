...
The agentic runtime: an orchestrator serves one request at a time over the user's memory, with six workers that own a slice of the tool catalogue. Every tool wraps an engine function in `code/buyorwait/`, so contract columns never depend on this package. Full rationale: `docs/AGENTIC.md`, decisions D13–D15.
